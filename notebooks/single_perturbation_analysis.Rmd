...
headline_labels <- c("gears" = "GEARS", "scgpt" = "scGPT", "scfoundation" = "scFoundation",
                     "lpm_selftrained" = "Linear Model", "lpm_pertEmb" = "Linear Model\n(pretrained)")
...
main_pl <- main_pl_data %>%
  ...
  ggplot(aes(x = method, y = l2)) +
    geom_hline(yintercept = 0, color = "black", linewidth = 0.2) +
    ggrastr::rasterize(ggbeeswarm::geom_quasirandom(color = alpha("#444444", 0.6), size = 0.6, stroke = 0, width = 0.3), dpi=600) +
    stat_summary(geom = "crossbar", fun = mean, color = "red") +
    facet_wrap(vars(dataset_name), scales = "free_x", labeller = as_labeller(dataset_labels), nrow = 1) +
    scale_x_discrete(labels = headline_labels, drop=FALSE) +
    scale_y_continuous(limits = c(0, NA), expand = expansion(add = c(0, 0.5))) +
    guides(x = guide_axis(angle = 90), color = "none") +
    labs(y = "Prediction error ($L_2$)") +
    theme(axis.title.x = element_blank(),
...
strat_plot <- strat_data_expr_rank %>%
  ...
  ggplot(aes(x = rank, y = dist_mean)) +
    ...
    scale_x_log10() +
    scale_y_continuous(limits = c(0, NA), expand = expansion(add = c(0, 0.5))) +
    scale_color_discrete(labels = str_replace(headline_labels, "\n", " ") |> magrittr::set_names(names(headline_labels)), drop=TRUE) +
...
comparison_plot_scgpt <- res_metrics %>%
  ...
  ggplot(aes(x = dataset_name, y = l2$mean)) +
    ...
    labs(y = "Prediction error ($L_2$)", 
         subtitle = paste0("\\textcolor{ggplotRed}{\\scalebox{1.3}{$\\bullet$}} scGPT vs.\n",
                           "\\textcolor{ggplotBlue}{\\scalebox{1.3}{$\\bullet$}} LPM with $\\matr{G}$ from scGPT")) +
    guides(x = guide_axis(angle = 90)) +
...
comparison_plot_scfoundation <- res_metrics %>%
  ...
  ggplot(aes(x = dataset_name, y = l2$mean)) +
    ...
    labs(y = "Prediction error ($L_2$)", 
         subtitle = paste0("\\textcolor{ggplotRed}{\\scalebox{1.3}{$\\bullet$}} scGPT vs.\n",
                           "\\textcolor{ggplotBlue}{\\scalebox{1.3}{$\\bullet$}} LPM with $\\matr{G}$ from scFoundation")) +
    guides(x = guide_axis(angle = 90)) +
    theme(axis.title.x = element_blank(), plot.title.position = "plot")
...
comparison_plot_gears <- res_metrics %>%
  ...
  ggplot(aes(x = dataset_name, y = l2$mean)) +
    ...
    scale_y_continuous(limits = c(2.8, 6.8), breaks = c(3, 4, 5, 6)) +
    labs(y = "Prediction error ($L_2$)", 
         subtitle = paste0("\\textcolor{ggplotRed}{\\scalebox{1.3}{$\\bullet$}} GEARS vs.\n",
                           "\\textcolor{ggplotBlue}{\\scalebox{1.3}{$\\bullet$}} LPM with $\\matr{P}$ from GEARS")) +
    guides(x = guide_axis(angle = 90)) +
    theme(axis.title.x = element_blank(), plot.title.position = "plot")
...
comparison_plot_lpm <- res_metrics %>%
  ...
  ggplot(aes(x = dataset_name, y = l2$mean)) +
    ...
    scale_y_continuous(limits = c(2.8, 6.8), breaks = c(3, 4, 5, 6)) +
    labs(y = "Prediction error ($L_2$)", 
         subtitle = paste0("\\textcolor{ggplotRed}{\\scalebox{1.3}{$\\bullet$}} LPM vs.\n",
                           "\\textcolor{ggplotBlue}{\\scalebox{1.3}{$\\bullet$}} LPM with $\\matr{P}$ from Replogle")) +
    guides(x = guide_axis(angle = 90)) +
    theme(axis.title.x = element_blank(), plot.title.position = "plot")
...
comparison_plot_lpm2 <- res_metrics %>%
  ...
  ggplot(aes(x = dataset_name, y = l2$mean)) +
    ...
    labs(y = "Prediction error ($L_2$)", 
         subtitle = paste0("\\textcolor{ggplotRed}{\\scalebox{1.3}{$\\bullet$}} LPM vs.\n",
                           "\\textcolor{ggplotBlue}{\\scalebox{1.3}{$\\bullet$}} LPM with random $\\matr{P}$")) +
    guides(x = guide_axis(angle = 90)) +
    theme(axis.title.x = element_blank(), plot.title.position = "plot")
...
plot_assemble(
  ...
  add_plot(strat_plot, x = 100, y = 42, width = 70, height = 47.5),
