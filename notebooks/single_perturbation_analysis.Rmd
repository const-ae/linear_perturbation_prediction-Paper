...
```{r}
tibble(x = c("a","c")) %>%
  mutate(x = factor(x, levels = c("a", "b","c", "D", "E"))) %>%
  bind_rows(tibble(x = "b", remove  =TRUE) %>% mutate(x = factor(x, levels = c("a", "b","c", "D", "E")))) %>%
  ggplot(aes(x = x, y = 1)) +
    geom_point(data = . %>% filter(is.na(remove))) +
    geom_blank(data = . %>% filter(remove)) +
    scale_x_discrete(drop=TRUE, limits = c("a", "b", "c", "D"))
```
...
headline_labels <- c("gears" = "GEARS", "scgpt" = "scGPT", "scfoundation" = "scFoundation",
                     "lpm_selftrained" = "LPM with $\\matr{G}$,$\\matr{P}$\nfrom training",
                     "lpm_k562PertEmb" = "LPM with $\\matr{P}$ from\nK562 Replogle",
                     "lpm_rpe1PertEmb" = "LPM with $\\matr{P}$ from\nRPE1 Replogle",
                     "lpm_pertEmb" = "LPM with $\\matr{P}$\nfrom Replogle")
...
strat_data_init <- contr_res %>%
  filter(train != "train") %>%
  tidylog::left_join(baselines, by = c("dataset_name", "seed")) %>%
  dplyr::select(-c(id, test_train_config_id, prediction_std)) %>%
  ...
  ggplot(aes(x = dataset_name, y = l2$mean)) +
    ...
    labs(y = "Prediction error ($L_2$)", 
         subtitle = paste0("\\textcolor{colorbrewerDarkOrange}{\\scalebox{1.3}{$\\bullet$}} scGPT vs.\n",
                           "\\textcolor{colorbrewerDarkPurple}{\\scalebox{1.3}{$\\bullet$}} LPM with $\\matr{G}$ from scGPT")) +
    guides(x = guide_axis(angle = 90)) +
...
comparison_plot_scfoundation <- res_metrics %>%
  ...
  ggplot(aes(x = dataset_name, y = l2$mean)) +
    ...
    labs(y = "Prediction error ($L_2$)", 
         subtitle = paste0("\\textcolor{colorbrewerDarkOrange}{\\scalebox{1.3}{$\\bullet$}} scGPT vs.\n",
                           "\\textcolor{colorbrewerDarkPurple}{\\scalebox{1.3}{$\\bullet$}} LPM with $\\matr{G}$ from scFoundation")) +
    guides(x = guide_axis(angle = 90)) +
    theme(axis.title.x = element_blank(), plot.title.position = "plot")
  ...
  ggplot(aes(x = dataset_name, y = l2$mean)) +
    ...
    labs(y = "Prediction error ($L_2$)", 
         subtitle = paste0("\\textcolor{colorbrewerDarkOrange}{\\scalebox{1.3}{$\\bullet$}} GEARS vs.\n",
                           "\\textcolor{colorbrewerDarkPurple}{\\scalebox{1.3}{$\\bullet$}} LPM with $\\matr{P}$ from GEARS")) +
    guides(x = guide_axis(angle = 90)) +
    theme(axis.title.x = element_blank(), plot.title.position = "plot")
...
comparison_plot_lpm <- res_metrics %>%
  ...
  ggplot(aes(x = dataset_name, y = l2$mean)) +
    ...
    scale_color_manual(values = contrast_colors) +  
    labs(y = "Prediction error ($L_2$)", 
         subtitle = paste0("\\textcolor{colorbrewerDarkOrange}{\\scalebox{1.3}{$\\bullet$}} LPM vs.\n",
                           "\\textcolor{colorbrewerDarkPurple}{\\scalebox{1.3}{$\\bullet$}} LPM with $\\matr{P}$ from Replogle")) +
    guides(x = guide_axis(angle = 90)) +
    theme(axis.title.x = element_blank(), plot.title.position = "plot")
...
comparison_plot_lpm2 <- res_metrics %>%
  ...
  ggplot(aes(x = dataset_name, y = l2$mean)) +
    ...
    labs(y = "Prediction error ($L_2$)", 
         subtitle = paste0("\\textcolor{colorbrewerDarkOrange}{\\scalebox{1.3}{$\\bullet$}} LPM vs.\n",
                           "\\textcolor{colorbrewerDarkPurple}{\\scalebox{1.3}{$\\bullet$}} LPM with random $\\matr{P}$")) +
    guides(x = guide_axis(angle = 90)) +
    theme(axis.title.x = element_blank(), plot.title.position = "plot")
...
```{r}
plot_assemble(
  add_text("(A) Linear Pretrained Model (LPM)", x = 2.7, y = 1, fontsize = font_size, vjust = 1, fontface = "bold"),
  add_text("(B) Foundation models for perturbation data", x = 95, y = 1, fontsize = font_size, vjust = 1, fontface = "bold"),
  add_graphic("../illustrations/model_comparisonArtboard 1.pdf", x = 0, y = 2, units = "mm"),
  
  
  add_text("(C) Single perturbation prediction error", x = 2.7, y = 39, fontsize = font_size, vjust = 1, fontface = "bold"),
  add_plot(main_pl, x = 0, y = 42, width = 100, height = 47.5),
  
  add_text("(D) Prediction error stratified by gene set size", x = 102, y = 39, fontsize = font_size, vjust = 1, fontface = "bold"),
  add_plot(strat_plot, x = 100, y = 42, width = 70, height = 45.5),
  
  add_text("(E) Plugging different embeddings into the LPM", x = 2.7, y = 92, fontsize = font_size, vjust = 1, fontface = "bold"),
  add_plot(comparison_plot_scgpt, x = 0, y = 96, width = 30, height = 42),
  add_plot(comparison_plot_scfoundation, x = 33, y = 96, width = 30, height = 42),
  add_plot(comparison_plot_gears, x = 68, y = 96, width = 30, height = 42),
  add_plot(comparison_plot_lpm,  x = 101, y = 96, width = 30, height = 42),
  add_plot(comparison_plot_lpm2, x = 134, y = 96, width = 30, height = 42),

  width = 170, height = 138, units = "mm", show_grid_lines = FALSE,
