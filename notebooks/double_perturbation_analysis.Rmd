...
```{r}
plot_assemble(
  add_text("(A) Quantile-Quantile plot of the deviation from the additive model", 
           x = 2.7, y = 1, fontsize = font_size, vjust = 1, fontface = "bold"),
...
#   left_join(expr_var_rank_df %>% dplyr::select(dataset_name, seed, gene_name, rank = var_rank) %>% filter(rank %in% sel_ranks), 
#             by = c("dataset_name", "seed", "gene_name")) %>%
#   arrange(rank) %>%
#   mutate(dist = sqrt(cumsum((prediction - observed)^2)),
#          .by = c(dataset_name, seed, method, perturbation))%>% 
